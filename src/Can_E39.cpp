...
//byte 7 - md_ind_lm_ist -- theoretical engine torque from air mass, excluding igntion angle (in %)
   ...
   can->Send(0x316, (uint32_t*)bytes, 8); //Send on CAN2
}
...
//Byte 7 - unused
   ...
   can->Send(0x329, (uint32_t*)bytes, 8); //Send on CAN2
}
...
void Can_E39::Msg43F()
{
    ...
    can->Send(0x43F, (uint32_t*)bytes,8); //Send on CAN2
}
...
//4000   7   111   #111####
   ...
   can->Send(0x545, (uint32_t*)bytes, 8); //Send on CAN2
}
