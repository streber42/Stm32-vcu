...
void Can_VAG::SendVAG100msMessage()
{
    ...
    Can::GetInterface(Param::GetInt(Param::veh_can))->Send(0x580, (uint32_t*)canData,8); //Send on CAN2
    seqCtr = (seqCtr + 1) & 0x3;
    ctr = (ctr + 1) & 0xF;
}
...
void Can_VAG::SendVAG10msMessage(uint16_t rpm)
{
   ...
   Can::GetInterface(Param::GetInt(Param::veh_can))->Send(0x280, (uint32_t*)canData, 8); //Send on CAN2

   //contains temperature, traction control light was on without the message, content doesnt
   //seem to matter.
   canData[3] = 0;
   canData[4] = 0;
   Can::GetInterface(Param::GetInt(Param::veh_can))->Send(0x288, (uint32_t*)canData, 8); //Send on CAN2
